...
class Conv(nn.Module):
    # Standard convolution, now LoRA-aware
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True, d=1, use_lora=False, lora_config=None):  # Thêm các tham số lora
        super(Conv, self).__init__()

        if use_lora:
            # Nếu sử dụng LoRA, khởi tạo lora.Conv2d
            # Lưu ý: lora.Conv2d từ LoRA-C không có sẵn tham số 'dilation' và 'padding', chúng ta cần truyền nó vào kwargs
            # Hoặc đơn giản là tính toán padding trước
            padding = autopad(k, p)
            ...
            # Nếu không, giữ nguyên nn.Conv2d
...
class DWConv(Conv):
    # Depthwise convolution, now LoRA-aware
    def __init__(self, c1, c2, k=1, s=1, act=True, use_lora=False, lora_config=None):  # Thêm tham số
        super().__init__(c1, c2, k, s, g=math.gcd(c1, c2), act=act, use_lora=use_lora, lora_config=lora_config)
...
class C3(nn.Module):
    # CSP Bottleneck with 3 convolutions, now LoRA-aware
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, use_lora=False, lora_config=None):
        super(C3, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1, use_lora=use_lora, lora_config=lora_config)
        self.cv2 = Conv(c1, c_, 1, 1, use_lora=use_lora, lora_config=lora_config)
        self.cv3 = Conv(2 * c_, c2, 1, use_lora=use_lora, lora_config=lora_config)
        self.m = nn.Sequential(*[Bottleneck(c_, c_, shortcut, g, e=1.0, use_lora=use_lora, lora_config=lora_config) for _ in range(n)])
